...
def main():
  ...
  extrasFileSource = serviceTemplate + '/matter_extras.yml'
  ...
  def mainRender(needsRender, menu, selection):
    ...
    if needsRender == 1:
      ...
      print(term.black_on_cornsilk4(term.center('IOTstack Python Matter Server')))
      print("")
      print(term.center(commonTopBorder(renderMode)))
      print(term.center(commonEmptyLine(renderMode)))
      print(term.center("{bv}   Select Python Matter Server                                                  {bv}".format(bv=specialChars[renderMode]["borderVertical"])))
      print(term.center(commonEmptyLine(renderMode)))
    ...
    if needsRender == 1:
      ...
      if not hideHelpText:
        if term.height < 32:
          ...
          print(term.center("{bv}      [Space] to select or deselect extras                                      {bv}".format(bv=specialChars[renderMode]["borderVertical"])))
          print(term.center("{bv}      [Up] and [Down] to move selection cursor                                  {bv}".format(bv=specialChars[renderMode]["borderVertical"])))
          print(term.center("{bv}      [H] Show/hide this text                                                   {bv}".format(bv=specialChars[renderMode]["borderVertical"])))
          print(term.center("{bv}      [Enter] to build and save extras list                                     {bv}".format(bv=specialChars[renderMode]["borderVertical"])))
  ...
  def loadAddonsMenu():
    global mainMenuList
    if os.path.exists(extrasFileSource):
      with open(r'%s' % extrasFileSource) as objExtrasListFile:
        extrasKnown = yaml.load(objExtrasListFile)
        knownExtrasList = extrasKnown["extrasList"]
        if os.path.exists("{serviceDir}{buildSettings}".format(serviceDir=serviceService, buildSettings=buildSettingsFileName)):
          with open("{serviceDir}{buildSettings}".format(serviceDir=serviceService, buildSettings=buildSettingsFileName)) as objSavedExtrasListFile:
            savedExtrasList = yaml.load(objSavedExtrasListFile)
            savedExtras = []

            try:
              savedExtras = savedExtrasList["extras"]
            except:
              print("Error: Loading saved extras selection. Please resave your selection.")
              input("Press Enter to continue...")

            for (index, extrasPath) in enumerate(knownExtrasList):
              if extrasPath in savedExtras:
                mainMenuList.append([extrasPath, { "checked": True }])
              else:
                mainMenuList.append([extrasPath, { "checked": False }])

        else: # No saved list
          for (index, extrasPath) in enumerate(knownExtrasList):
            if os.path.exists(extrasPath):
              mainMenuList.append([extrasPath, { "checked": True }])
            else:
              mainMenuList.append([extrasPath, { "checked": False }])
      ...
      print("Error: '{extrasFile}' file doesn't exist.".format(extrasFile=extrasFileSource))
      input("Press Enter to continue...")
  ...
  def saveAddonList():
    try:
      ...
      pythonMatterServerYamlExtrasList = {
        "version": "1",
        "application": "IOTstack",
        "service": "pythonMatterServer",
        "comment": "Build Settings",
        "extras": []
      }
      for (index, addon) in enumerate(mainMenuList):
        if addon[1]["checked"]:
          pythonMatterServerYamlExtrasList["extras"].append(addon[0])

      with open("{serviceDir}{buildSettings}".format(serviceDir=serviceService, buildSettings=buildSettingsFileName), 'w') as outputFile:
        yaml.dump(pythonMatterServerYamlExtrasList, outputFile)

    except Exception as err: 
      print("Error saving Python Matter Server list", currentServiceName)
      print(err)
      return False
    global hasRebuiltExtrasSelection
    hasRebuiltExtrasSelection = True
    return True
